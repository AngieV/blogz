...
def getCurrentWatchlist():
    ...
    myWatchlist = [ "Star Wars", "Minions", "Freaky Friday", "My Favorite Martian" ]
    return myWatchlist
...
class AddMovie(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if (not(new_movie) or not(new_movie.strip()) == ""):
            error = "You did not give me a Movie to add :-("
            self.redirect("/?error=" + error)

        # TODO 3
        # if the user wants to add a terrible movie, redirect and yell at them
        if new_movie in terrible_movies:
            error = "Trust me, you do not want to waste your time watching that movie!"
            self.redirect("/?error=" + error)
        ...
        #getCurrentWatchlist().add(new_movie) 
        sentence = "<p><strong>" + new_movie + "</strong> has been added to your Watchlist!</p>"
